...
public interface UserServicesSpecializationsRepository extends JpaRepository<UserServicesSpecializations, Long> {
    ...
    @Query("select distinct (uss.service) from UserServicesSpecializations uss where uss.specialization = ?1 and uss.service.isActive=true")
    List<ServiceType> findAllActiveServicesForSelectedSpecialization(Specialization specialization);
